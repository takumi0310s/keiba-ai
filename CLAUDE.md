...
- ★**買い目構造backtest = 全構造NO-GO・現行7点が最良級 (6/12, docs §7.8)**★: V15 OOF(leak-free v2)で三連複/三連単フォーメーション6構造(m事前固定)を設計2020-22/評価2023-25で比較 → **対F0(現行7点)補正後CI生存ゼロ**。広げる構造(ながし10点/二軸14点)はwinsorize後**有意悪化**、切り強化5点は+3.0ptだが有意でなくmaxDD増、三連単はδ負で検証不能(控除率27.5%)。根拠=V15のtop6収納率59.8%だが**払戻金ベース捕捉率13.5%**=高配当はモデル圏外 → 買い目組み替えでは効率改善せず。改善路線は戦略層(レース選択)とモデル層。paper追加なし・本番不変。
...
- ✅**DEAD データ 8源 = 全修理済 (6/12 統合タスク、tag pre_fix_0612)**: KTA(アーカイブindex依存→直接日付フェッチ jrdb_daily_fix_fetch.py 新設+3:00 job配線)・KKA(extracted非空スキップ→同)・TYB/SKB(LF bat起因→再構築 552k/551k行)・SRB/SR(ローカルSED lzhから復元)・CYB(スキーマ混在→bulk全量再構築 553k行)・netkeiba_training_eval(converterキー不一致→マップ修正+ゾンビ1,594行除去、2026=5,854行)。全13 JRDB源 content-fresh(6/7)。詳細=docs/FABLE_SWEEP_LOG.md 統合タスク章
- ✅**app.py 表示修理済 (6/12 承認済・予測ロジック不変)**: discovery に _NON_PRODUCTION_MODELS(v20/v21/v22系除外) → Pattern Aスロット=v15復元 / バッジ動的化(LGB+XGB 2-MODEL、AUC(train self-eval)表記) / track_record.csv 再構築(766R/Σ-29,450=台帳一致) / phase3配線(daily_results→log_phase3、8戦略paper結果記録が初稼働)
- 🟡**要承認の残件**: 142.6%(v12)基準の恒常誤DANGER 3ファイル(roi_monitor/weekly_report/roi_analysis) / schtask 7件未登録(register bat 3本を管理者実行要) / TYBリアルタイム(tyokuzen path)実装
