...
def precompute_folds(df, features_all_possible):
    """Pre-compute fold data (train/valid/test splits + sire re-encoding) for all folds.
    Returns dict of {year: {'X_tr': ..., 'y_tr': ..., 'X_va': ..., 'y_va': ..., 'X_test': ..., 'y_test': ...}}
    with all possible features as columns.
    """
    folds = {}
    for test_year in TEST_YEARS:
        ...
        # Re-encode sires using only training data
        train_df_tmp = df[train_mask]
        sire_counts = train_df_tmp['father'].value_counts()
        top_sires = sire_counts.head(N_TOP_SIRE).index.tolist()
        sire_map = {s: i for i, s in enumerate(top_sires)}
        sire_enc = df['father'].map(sire_map).fillna(N_TOP_SIRE).astype(int)

        bms_counts = train_df_tmp['bms'].value_counts()
        top_bms = bms_counts.head(N_TOP_SIRE).index.tolist()
        bms_map = {s: i for i, s in enumerate(top_bms)}
        bms_enc = df['bms'].map(bms_map).fillna(N_TOP_SIRE).astype(int)

        # Build feature matrix with re-encoded sires
        feat_data = {}
        for f in features_all_possible:
            if f == 'sire_enc':
                feat_data[f] = sire_enc.values.astype(np.float32)
            elif f == 'bms_enc':
                feat_data[f] = bms_enc.values.astype(np.float32)
            elif f == 'sire_dist':
                feat_data[f] = (sire_enc * 10 + df['dist_cat']).values.astype(np.float32)
            elif f == 'sire_surface':
                feat_data[f] = (sire_enc * 10 + df['surface_enc']).values.astype(np.float32)
            elif f == 'bms_dist':
                feat_data[f] = (bms_enc * 10 + df['dist_cat']).values.astype(np.float32)
            elif f in df.columns:
                feat_data[f] = pd.to_numeric(df[f], errors='coerce').fillna(0).values.astype(np.float32)
            else:
                feat_data[f] = np.zeros(len(df), dtype=np.float32)

        # Build full feature matrix
        X_all = np.column_stack([feat_data[f] for f in features_all_possible])
        y_all = df['target'].values

        # Split train into tr/va
        train_idx = train_mask.values
        test_idx = test_mask.values
        dates = df['date_num'].values
        train_dates = dates[train_idx]
        cutoff = np.quantile(train_dates, 0.85)
        tr_sub = train_idx.copy()
        va_sub = train_idx.copy()
        tr_sub[train_idx] = dates[train_idx] < cutoff
        va_sub[train_idx] = dates[train_idx] >= cutoff
        # Zero out non-train rows
        tr_sub[~train_idx] = False
        va_sub[~train_idx] = False
        ...
        print(f"  Fold {test_year}: tr={tr_sub.sum()}, va={va_sub.sum()}, test={test_idx.sum()}")

    return folds


def walk_forward_auc_fast(folds, features_all, feature_indices):
    """Run walk-forward using pre-computed folds and feature column indices."""
    fold_aucs = {}
    for test_year, fold in folds.items():
        X_tr = fold['X_tr'][:, feature_indices]
        y_tr = fold['y_tr']
        X_va = fold['X_va'][:, feature_indices]
        y_va = fold['y_va']
        X_test = fold['X_test'][:, feature_indices]
        y_test = fold['y_test']

        feat_names = [features_all[i] for i in feature_indices]
        model = train_lgb_fold(X_tr, y_tr, X_va, y_va, feat_names)
...
def compute_avg_finish_5r(df):
    """1. Average finish position over last 5 races."""
...
def compute_jockey_dist_wr(df):
    """2. Jockey win rate at specific distance category."""
...
def compute_horse_course_top3r(df):
    """4. Horse's top3 rate at specific course."""
...
def compute_prev_prize_rank(df):
    """5. Previous race prize rank ratio."""
...
def compute_class_change(df):
    """6. Class change from previous race."""
...
def compute_horse_season_wr(df):
    """7. Horse's historical top3 rate in current season."""
...
def compute_trainer_jockey_combo_wr(df):
    """8. Trainer x Jockey combination win rate."""
...
def compute_horse_avg_prize_3r(df):
    """9. Average prize money in last 3 races (log)."""
    ...
    df['horse_avg_prize_3r'] = np.log1p(df[['prev_prize_1', 'prev_prize_2', 'prev_prize_3']].mean(axis=1))
    df = df.drop(columns=['prev_prize_1', 'prev_prize_2', 'prev_prize_3'], errors='ignore')
    return df


def compute_sire_age_wr(df):
    """10. Sire progeny win rate at horse's age group."""
...
CANDIDATES = {
    'avg_finish_5r': {'compute': compute_avg_finish_5r, 'columns': ['avg_finish_5r']},
    'jockey_dist_wr': {'compute': compute_jockey_dist_wr, 'columns': ['jockey_dist_wr']},
    'days_since_last_race': {'compute': compute_days_since_last, 'columns': ['days_since_last_race']},
    'horse_course_top3r': {'compute': compute_horse_course_top3r, 'columns': ['horse_course_top3r']},
    'prev_prize_rank': {'compute': compute_prev_prize_rank, 'columns': ['prev_prize_rank']},
    'class_change': {'compute': compute_class_change, 'columns': ['class_change']},
    'horse_season_wr': {'compute': compute_horse_season_wr, 'columns': ['horse_season_wr']},
    'trainer_jockey_combo_wr': {'compute': compute_trainer_jockey_combo_wr, 'columns': ['trainer_jockey_combo_wr']},
    'horse_avg_prize_3r': {'compute': compute_horse_avg_prize_3r, 'columns': ['horse_avg_prize_3r']},
    'sire_age_wr': {'compute': compute_sire_age_wr, 'columns': ['sire_age_wr']},
}
...
def main():
    ...
    # Build the full features list (baseline + all candidates)
    all_candidate_cols = []
    for info in CANDIDATES.values():
        all_candidate_cols.extend(info['columns'])
    features_all = features_base + all_candidate_cols
    print(f"Total features (baseline + candidates): {len(features_all)}")

    # === Pre-compute folds (expensive but done once) ===
    print("\n--- Pre-computing fold data ---")
    t0 = time.time()
    folds = precompute_folds(df, features_all)
    print(f"  Fold pre-computation done ({time.time()-t0:.0f}s)")

    # Feature index lookup
    feat_to_idx = {f: i for i, f in enumerate(features_all)}
    base_indices = np.array([feat_to_idx[f] for f in features_base])
    ...
    print("=" * 70)
    t0 = time.time()
    baseline_auc, baseline_folds = walk_forward_auc_fast(folds, features_all, base_indices)
    ...
    for name, info in CANDIDATES.items():
        cols = info['columns']
        candidate_indices = np.array([feat_to_idx[c] for c in cols])
        test_indices = np.concatenate([base_indices, candidate_indices])
        print(f"\n  Testing: {name} (cols={cols})")
        t0 = time.time()
        test_auc, test_folds = walk_forward_auc_fast(folds, features_all, test_indices)
        delta = test_auc - baseline_auc
        adopted = delta > 0.0001
    ...
    if adopted_features:
        ...
        combined_indices = np.concatenate([base_indices, np.array([feat_to_idx[c] for c in combined_cols])])
        t0 = time.time()
        combined_auc, combined_folds = walk_forward_auc_fast(folds, features_all, combined_indices)
    ...
    for name, r in results.items():
        serializable_results[name] = {
            'wf_auc': float(r['wf_auc']),
            'delta': float(r['delta']),
            'adopted': bool(r['adopted']),
            'year_aucs': {str(y): float(a) for y, a in r['year_aucs'].items()},
        }

    output = {
        'baseline_wf_auc': round(float(baseline_auc), 5),
        'baseline_year_aucs': {str(y): round(float(a), 5) for y, a in baseline_folds.items()},
        'candidates': serializable_results,
        'best_features': adopted_features,
        'final_wf_auc': round(float(combined_auc), 5),
        'final_year_aucs': {str(y): round(float(a), 5) for y, a in combined_folds.items()},
