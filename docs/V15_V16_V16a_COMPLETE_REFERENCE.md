# V15 / V16 / V16a 完全リファレンス
> 作成: 2026-05-28 Session #92
> 目的: 新規チャットでもゼロから完全理解できる詳細仕様書

---

## 0. 全体概要

| モデル | ファイル | 特徴量数 | WF AUC | 状態 |
|--------|---------|---------|--------|------|
| **V15** | `keiba_model_v15_central.pkl.gz` | **145** | **0.8678** | **本番稼働** |
| V16 | `models/v16_ability_candidate.pkl.gz` | **137** | **0.8677** | paper shadow |
| V16a | `models/v16a_candidate.pkl.gz` | **144** | **0.8674** | paper shadow (NO-GO) |

- **WF AUC の真値**: genuine WF LGB+XGB (fold ごと retrain)。stored `.pkl.auc = 0.8939` は LGB train-set self-eval (in-sample) = LEAKY 参照不可
- **アーキテクチャ**: 全モデル LGB + XGB 2-model ensemble。MLP/FT-Transformer/IntraRace は pkl.gz 未保存 → production inference は LGB+XGB のみ
- **本番投票**: V15 のみ。V16/V16a は paper shadow (投票なし)

---

## 1. V15 本番モデル

### 1-1. pkl.gz 内部構造

```python
{
    'model':            LightGBM Booster,
    'xgb_model':        XGBoost Booster,
    'features':         [145 feature names],   # ← 推論時はこのリストを使う
    'ensemble_weights': {'lgb': 0.5036, 'xgb': 0.4964, 'mlp': 0},
    'version':          'v15',
    'auc':              0.8939,   # ← LGB train-set self-eval (in-sample) ★参照禁止★
    'leak_free':        True,
    'leak_pattern':     'A',
    'sire_map':         {sire_name: int, ...},   # 血統エンコード用
    'bms_map':          {bms_name: int, ...},
    'course_map':       {course_name: int, ...},
    'mlp_model':        None,
    'mlp_scaler':       None,
    'trained_at':       '2026-...',
}
```

### 1-2. 特徴量完全リスト (145個、順番通り)

```
# ── 基本属性 ──────────────────────────────────────────
 1  weight_carry          # 斤量
 2  age                   # 馬齢
 3  distance              # 距離 (m)
 4  course_enc            # 競馬場コード (int)
 5  surface_enc           # 馬場種別: 芝=0, ダ=1
 6  sex_enc               # 性別: 牡=0, 牝=1, セ=2
 7  num_horses_val        # 頭数
 8  horse_num             # 馬番 (1-18)

# ── 枠・番号系 ────────────────────────────────────────
 9  bracket               # 枠番 (1-8)
10  jockey_wr_calc        # 騎手勝率 (expanding)
11  jockey_course_wr_calc # 騎手×コース勝率
12  trainer_top3_calc     # 調教師複勝率 (expanding)

# ── 前走成績 ─────────────────────────────────────────
13  prev_finish           # 前走着順
14  prev_last3f           # 前走上がり3F
15  prev_pass4            # 前走通過4角
16  prev_prize            # 前走賞金
17  prev2_finish          # 2走前着順
18  prev3_finish          # 3走前着順
19  avg_finish_3r         # 直近3走平均着順
20  best_finish_3r        # 直近3走最高着順
21  finish_trend          # 着順トレンド (改善/悪化)
22  top3_count_3r         # 直近3走複勝圏内回数
23  avg_last3f_3r         # 直近3走平均上がり3F
24  prev2_last3f          # 2走前上がり3F

# ── 距離・条件変化 ────────────────────────────────────
25  dist_change           # 前走比距離変化 (m)
26  dist_change_abs       # 距離変化絶対値
27  rest_days             # 休養日数
28  rest_category         # 休養カテゴリ (0-4)
29  sire_enc              # 父馬エンコード
30  bms_enc               # 母父エンコード
31  dist_cat              # 距離カテゴリ (sprint/mile/etc)
32  age_sex               # 齢×性別 交互作用
33  season                # 季節 (0-3)
34  age_season            # 齢×季節
35  horse_num_ratio       # 馬番/頭数 (外枠率)
36  bracket_pos           # 枠位置 (inner/middle/outer)
37  carry_diff            # 斤量差 (馬平均との差)
38  age_group             # 齢グループ (2/3/4/5+)
39  surface_dist_enc      # 馬場×距離カテゴリ
40  course_surface        # コース×馬場
41  location_enc          # 地域エンコード
42  is_nar                # NAR (地方) フラグ (V15では常に0)

# ── オッズ系 (V16で除外) ─────────────────────────────
43  prev_odds_log         # 前走オッズ log
...
116 oz_tansho_base_log    # 基準単勝オッズ log
117 oz_fukusho_base_log   # 基準複勝オッズ log
118 oz_base_pop_rank      # 基準人気順位
119 odds_change_rate      # 当日オッズ変化率
120 pop_rank_change       # 人気順位変化
121 odds_sharp_drop       # オッズ急落フラグ
...
131 paci_ninki_idx        # PACI 人気指数 (odds-derived)

# ── 調教 ─────────────────────────────────────────────
44  training_time_filled  # 調教タイム (補完済)
45  has_training          # 調教データ有無フラグ
46  training_per_dist     # 距離あたり調教タイム
47  jockey_surface_wr     # 騎手×馬場勝率
54  wood_best_4f_filled   # 坂路最高4F
55  has_wood_training     # 坂路データ有無
60  wood_count_2w         # 2週間坂路本数
61  sakaro_best_4f_filled # 坂路4F
62  sakaro_best_3f_filled # 坂路3F
63  has_sakaro_training   # 坂路データ有無
64  total_training_count  # 総調教本数
68  index_max_filled      # 調教指数最高値
69  index_run1_filled     # 最終追い調教指数
70  index_avg5_filled     # 直近5本平均指数
71  time_1f_last_filled   # 最終1F
72  training_intensity_enc # 調教強度カテゴリ

# ── キャリア統計 ─────────────────────────────────────
48  horse_career_races    # 通算出走数
49  horse_career_wr       # 通算勝率
50  horse_career_top3r    # 通算複勝率
65  horse_dist_top3r      # 距離別複勝率
66  horse_surface_top3r   # 馬場別複勝率
67  frame_course_dist_wr  # コース×距離×枠 勝率

# ── 血統 ──────────────────────────────────────────────
51  sire_surface_wr       # 父馬×馬場勝率
52  sire_dist_wr          # 父馬×距離勝率
53  bms_surface_wr        # 母父×馬場勝率
73  sire_shinba_top3r     # 父馬新馬複勝率
74  pci                   # PACI 総合指数

# ── 前走ペース詳細 ────────────────────────────────────
56  prev_race_first3f     # 前走前半3F
57  prev_race_last3f      # 前走後半3F
58  prev_race_pace_diff   # 前走ペース差

# ── JRDB 指数 (30個) ─────────────────────────────────
75  jrdb_idm              # IDM 指数
76  jrdb_training_idx     # 調教指数
77  jrdb_stable_idx       # 厩舎指数
78  jrdb_composite_idx    # 総合指数
79  jrdb_upset_idx        # 波乱指数
80  jrdb_ten_idx_pred     # 出走当日テン指数予測
81  jrdb_pace_idx_pred    # ペース指数予測
82  jrdb_agari_idx_pred   # 上がり指数予測
83  jrdb_position_idx_pred # ポジション指数予測
84  jrdb_class_code       # クラスコード
85  jrdb_rise_code        # 上昇度コード
86  jrdb_heavy_apt        # 重馬場適性
87  jrdb_hoof_code        # 蹄コード
88  jrdb_ranch_rank       # 牧場ランク
89  jrdb_stable_rank      # 厩舎ランク
90  jrdb_training_arrow   # 調教矢印
91  jrdb_stable_eval      # 厩舎評価
92  jrdb_running_style    # 脚質コード
93  jrdb_dist_apt         # 距離適性
94  jrdb_prev_idm         # 前走IDM
95  jrdb_prev_track_bias  # 前走馬場バイアス
96  jrdb_prev_interference # 前走障害 (不利)
97  jrdb_prev_late_start  # 前走出遅れ
98  jrdb_prev_pace_idx    # 前走ペース指数
99  jrdb_prev_rise_code   # 前走上昇度
100 jrdb_oikiri_idx       # 追い切り指数
101 jrdb_ten_time_idx     # テンタイム指数
102 jrdb_shimai_time_idx  # 終い時間指数
103 jrdb_cid_idx          # CID指数
104 jrdb_ls_idx           # LS指数
105 jrdb_kta_idm          # 近走平均IDM
106 jrdb_kta_ten_pred     # 近走テン予測
107 jrdb_kta_agari_pred   # 近走上がり予測
108 jrdb_ze_idm_avg       # 全走平均IDM
109 jrdb_ze_ten_avg       # 全走平均テン
110 jrdb_ze_agari_avg     # 全走平均上がり
111 jrdb_ze_furi_count    # 振り返りカウント
112 jrdb_tb_homestr_inner # 内ラチ直線バイアス
113 jrdb_dam_rensho_avg   # 母馬連勝平均
114 jrdb_bms_rensho_avg   # 母父連勝平均
115 stable_comment_score  # 厩舎コメントスコア (-3〜+3)

# ── 体重トレンド ──────────────────────────────────────
122 weight_ma5            # 馬体重 5走移動平均
123 weight_trend          # 体重トレンド
124 weight_peak_diff      # ピーク比体重差

# ── PACI (netkeiba 会員限定) ──────────────────────────
125 paci_manken_idx       # 万券指数
126 paci_goal_rank        # ゴール順位予測
127 paci_dochu_rank       # 道中順位予測
128 paci_goal_diff        # ゴール差
129 paci_jockey_exp_wr    # 騎手経験勝率
130 paci_jockey_exp_3rd   # 騎手経験複勝率
                          # paci_ninki_idx は V16 で除外 (#131)

# ── V15 新特徴量 (10個, v14→v15 追加) ───────────────
132 jockey_horse_rides    # 騎手×馬 騎乗数
133 jockey_horse_wr       # 騎手×馬 勝率
134 jockey_horse_top3r    # 騎手×馬 複勝率
135 jockey_change         # 騎手交代フラグ
136 jockey_change_to_top  # 人気騎手への交代フラグ
137 transport_distance_km # 輸送距離 (km)
138 is_long_transport     # 長距離輸送フラグ (>100km)
139 course_renovated      # コース改修フラグ (京都等)
140 post_renovation_flag  # 改修後フラグ
141 gaisha_rank           # 外厩ランク

# ── PACI Tier B (4個) ────────────────────────────────
142 paci_sogo_mark        # PACI 総合マーク
143 paci_idm_mark         # PACI IDM マーク
144 paci_jockey_mark      # PACI 騎手マーク
145 paci_train_mark       # PACI 調教マーク
```

### 1-3. 推論コード (predict_core.py)

```python
def predict_race(df, model_data, odds_available=False, race_info=None):
    use_features = model_data['features']         # 145列名リスト
    m_lgb = model_data['model']                   # LightGBM Booster

    X_full = df.reindex(columns=use_features).fillna(0).values  # (N, 145)

    # LGB は booster に保存された特徴量数に合わせてスライス
    n_lgb = m_lgb.num_feature()  # 通常 145 (または truncate で 124 等)
    X = X_full[:, :n_lgb]

    p_lgb = m_lgb.predict(X)                      # (N,) → 複勝確率

    if 'xgb_model' in model_data and model_data['xgb_model']:
        p_xgb = model_data['xgb_model'].predict(xgb.DMatrix(X))
        w = model_data['ensemble_weights']         # lgb=0.5036, xgb=0.4964
        ai_scores = w['lgb'] * p_lgb + w['xgb'] * p_xgb
    else:
        ai_scores = p_lgb

    df['スコア'] = ai_scores
    return df.sort_values('スコア', ascending=False).reset_index(drop=True)
```

### 1-4. 精度指標 (真値)

| 指標 | 値 | 備考 |
|------|-----|------|
| stored `.pkl.auc` | 0.8939 | LGB train-set self-eval ★LEAKY★ |
| genuine WF 6-fold (2020-25) LGB+XGB | **0.8678** | ★ 真のベースライン ★ |
| genuine WF 5-fold (2021-25) LGB+XGB | 0.8695 | |
| Grid 4-model 5-fold mean | 0.8858 | FT/IR 含む、pkl 未保存 |
| 累計運用 ROI | 98.34% / ¥-6,920 | n=596 (≤2026-05-17) |
| 95% CI | [66.33%, 138.05%] | 100% 含む → 有意勝ち無し |

---

## 2. V16 候補モデル

### 2-1. 設計思想
オッズ・人気系 8 features を完全除外した「能力のみモデル」。  
**根拠**: `paci_ninki_idx` の LGB feature importance が 16.93% → 実態はオッズの proxy。能力特徴量だけで同等性能なら人気乖離馬を正しく評価できる。

### 2-2. 除外特徴量 (`ODDS_FEATURES_REMOVE`)

```python
ODDS_FEATURES_REMOVE = [
    'paci_ninki_idx',       # PACI 人気指数 (gain 16.93%)
    'odds_change_rate',     # 当日オッズ変化率
    'odds_sharp_drop',      # オッズ急落フラグ
    'oz_base_pop_rank',     # 基準人気順位
    'oz_fukusho_base_log',  # 基準複勝オッズ log
    'oz_tansho_base_log',   # 基準単勝オッズ log
    'pop_rank_change',      # 人気順位変化
    'prev_odds_log',        # 前走オッズ log (#43)
]
# V15 145 - 8 = V16 137 features
```

### 2-3. pkl.gz 内部構造

```python
{
    'version':          'v16_ability_candidate',
    'model':            LightGBM Booster,
    'xgb_model':        XGBoost Booster,
    'ensemble_weights': {'lgb': 0.5, 'xgb': 0.5, 'mlp': 0},
    'features':         [137 feature names],
    'n_features':       137,
    'wf_auc_mean':      0.8677,   # ← genuine WF 真値
    'wf_results':       [...],    # 年度別詳細
    'removed_features': [8 odds features],
    'leak_free':        True,
    'leak_pattern':     'A',
    'is_live':          False,
    'is_candidate':     True,
    'v15_baseline_wf_auc': 0.8678,
}
```

### 2-4. WF AUC 詳細

| 年度 | LGB | XGB | ENS |
|------|-----|-----|-----|
| 2021 | 0.8628 | 0.8640 | 0.8643 |
| 2022 | 0.8656 | 0.8666 | 0.8670 |
| 2023 | 0.8670 | 0.8682 | 0.8684 |
| 2024 | 0.8691 | 0.8700 | 0.8704 |
| 2025 | 0.8671 | 0.8681 | 0.8684 |
| **平均** | | | **0.8677** |

**vs V15**: delta = -0.0001 (実質同等)

### 2-5. race_auto_notify.py での V16 スコア計算

```python
# ファイル: tools/race_auto_notify.py

_cached_v16_model = None  # lazy-load cache

def _load_v16_model():
    """V16 ability-only candidate をロード (lazy)。失敗時は {} を返す。"""
    global _cached_v16_model
    if _cached_v16_model is not None:
        return _cached_v16_model
    v16_path = os.path.join(BASE_DIR, 'models', 'v16_ability_candidate.pkl.gz')
    with gzip.open(v16_path, 'rb') as f:
        _cached_v16_model = pickle.load(f)
    return _cached_v16_model

def _get_v16_scores(df, v16_data) -> dict:
    """返却: {馬番(int): score(float)}"""
    feat16 = v16_data['features']                      # 137個
    m_lgb  = v16_data['model']
    m_xgb  = v16_data.get('xgb_model')
    w      = v16_data.get('ensemble_weights', {'lgb': 0.5, 'xgb': 0.5})

    df_copy = df.copy()
    for f in feat16:                                   # 欠損 feature を 0 補完
        if f not in df_copy.columns:
            df_copy[f] = 0.0
        df_copy[f] = pd.to_numeric(df_copy[f], errors='coerce').fillna(0)

    X = df_copy[feat16].values
    p_lgb = m_lgb.predict(X)
    p_xgb = m_xgb.predict(xgb.DMatrix(X)) if m_xgb else p_lgb
    scores = w['lgb'] * p_lgb + w['xgb'] * p_xgb

    return {int(row['馬番']): float(scores[i])
            for i, (_, row) in enumerate(df_copy.iterrows())}

def _save_v16_paper_log(race_id, date_str, df, v16_scores, odds_dict):
    """V16 予測を data/v16_paper_log/YYYYMMDD.json に追記"""
    log_file = Path(BASE_DIR) / 'data' / 'v16_paper_log' / f'{date_str}.json'
    # ...  v15_top3, v16_top3, scores, odds を JSON list に append
```

**呼び出し箇所** (predict_and_notify 関数内):
```python
_v16_scores = {}
_v16_data = _load_v16_model()
if _v16_data and 'features' in _v16_data:
    _v16_scores = _get_v16_scores(df, _v16_data)

if _v16_scores:
    _save_v16_paper_log(race_id, date_str, df, _v16_scores, odds_dict)

# V16a paper shadow (全 CANDIDATE_MODELS)
_v15_preds_ps = [{'horse_num': int(row['馬番']), 'score': float(row.get('スコア', 0))}
                 for _, row in df.iterrows()]
_shadow_res = run_paper_shadow_comparison(race_id, df, _v15_preds_ps, race_info=rinfo)
log_paper_shadow(_shadow_res)  # → data/paper_shadow_log/shadow_YYYYMMDD.jsonl

# Discord 通知に V16 スコアを組み込む
title, msg, color = build_rich_bet_message(
    ..., v16_scores=_v16_scores or None)
```

---

## 3. V16a 候補モデル

### 3-1. 設計思想
V16 (オッズ除去版) に「オッズ非依存の能力特徴量」4グループ (7 features) を追加。  
全て expanding window (cumsum - current) でリークフリー + Bayesian smoothing。

### 3-2. 追加特徴量 (7個)

#### 3-2-1. Jockey × Trainer Combo
```python
V16A_FEATURE_NAMES[:2] = ['jockey_trainer_top3_rate_exp', 'jockey_trainer_n_exp']

# 計算 (train/features_v16a.py)
df = df.sort_values('date_num')  # 日付昇順ソート必須
jt_key = ['jockey_id', 'trainer_id']
df['_jt_cumcnt']  = df.groupby(jt_key).cumcount()             # 当該レース"前"の回数
df['_jt_cumtop3'] = df.groupby(jt_key)['is_top3'].cumsum() - df['is_top3']  # 当該レース"前"の複勝数

_JT_PRIOR_TOP3 = 0.33  # 事前分布: 全体複勝率 ~33%
_JT_ALPHA = 30         # smoothing 強度 (N=30 で完全フラット)

df['jockey_trainer_n_exp'] = df['_jt_cumcnt'].astype(float)
df['jockey_trainer_top3_rate_exp'] = (
    (df['_jt_cumtop3'] + _JT_PRIOR_TOP3 * _JT_ALPHA) /
    (df['_jt_cumcnt'] + _JT_ALPHA)
)
```

#### 3-2-2. 馬体重 Slope & Std (直近5走)
```python
V16A_FEATURE_NAMES[2:4] = ['horse_weight_slope_5', 'horse_weight_std_5']

# lag1 (直前走) 〜 lag5 (5走前) を horse_id 別に取得 (現走 = current は除外)
lags = {k: df.groupby('horse_id')['horse_weight'].shift(k) for k in range(1, 6)}
hw_mat = pd.concat(lags, axis=1).values  # shape (N, 5)

for each row:
    valid = ~np.isnan(row)
    if valid.sum() < 2: continue
    # OLS slope (x: lag5=oldest, lag1=newest で昇順)
    x = np.array([5, 4, 3, 2, 1])[valid]
    y = row[valid]
    slopes[i] = cov(x, y) / var(x)   # 正=体重増加トレンド
    stds[i]   = y.std()

# fillna: slope=0.0 (中立), std=5.0 (典型的変動)
```

#### 3-2-3. 正規化パフォーマンス (ELO proxy)
```python
V16A_FEATURE_NAMES[4] = 'horse_perf_score_exp'

_PERF_PRIOR = 0.33   # 事前: 3頭に1頭的中
_PERF_ALPHA = 10     # smoothing 強度 (N=10 で完全フラット)

denom = (df['num_horses'] - 1).clip(lower=1)
df['_norm_finish'] = ((df['num_horses'] - df['finish']) / denom).clip(0, 1)
# 意味: 1位=1.0, 最下位=0.0 の連続スコア

g = df.groupby('horse_id')
df['_pf_cumcnt'] = g.cumcount()
df['_pf_cumsum'] = g['_norm_finish'].cumsum() - df['_norm_finish']  # 当該除外

df['horse_perf_score_exp'] = (
    (df['_pf_cumsum'] + _PERF_PRIOR * _PERF_ALPHA) /
    (df['_pf_cumcnt'] + _PERF_ALPHA)
)
```

#### 3-2-4. 枠番バイアス
```python
V16A_FEATURE_NAMES[5:7] = ['gate_bias_top3_exp', 'gate_bias_n_exp']

_GATE_PRIOR_TOP3 = 0.33
_GATE_ALPHA = 50    # smoothing 強度 (N=50 で完全フラット)

df['_dist_bin'] = (df['distance'] // 400) * 400   # 400m刻みバケット
gate_key = ['course', '_dist_bin', 'surface', 'umaban']

df['_gate_cumcnt']  = df.groupby(gate_key).cumcount()
df['_gate_cumtop3'] = df.groupby(gate_key)['is_top3'].cumsum() - df['is_top3']

df['gate_bias_n_exp'] = df['_gate_cumcnt'].astype(float)
df['gate_bias_top3_exp'] = (
    (df['_gate_cumtop3'] + _GATE_PRIOR_TOP3 * _GATE_ALPHA) /
    (df['_gate_cumcnt'] + _GATE_ALPHA)
)
```

### 3-3. デフォルト値 (unknown key 時)

```python
V16A_DEFAULTS = {
    'jockey_trainer_top3_rate_exp': 0.33,  # prior そのまま
    'jockey_trainer_n_exp':         0.0,
    'horse_weight_slope_5':         0.0,   # 中立 (増減なし)
    'horse_weight_std_5':           5.0,   # 典型的変動
    'horse_perf_score_exp':         0.33,  # prior そのまま
    'gate_bias_top3_exp':           0.33,  # prior そのまま
    'gate_bias_n_exp':              0.0,
}
```

### 3-4. pkl.gz 内部構造

```python
{
    'version':          'v16a_candidate',
    'model':            LightGBM Booster,
    'xgb_model':        XGBoost Booster,
    'ensemble_weights': {'lgb': 0.5, 'xgb': 0.5, 'mlp': 0},
    'features':         [144 feature names],
    'n_features':       144,
    'wf_auc_mean':      0.8674,
    'wf_results':       [...],
    'new_features':     ['jockey_trainer_top3_rate_exp', ...],   # 7件
    'new_feature_defaults': V16A_DEFAULTS,
    'leak_free':        True,
    'is_live':          False,
    'is_candidate':     True,
    'v15_baseline_wf_auc': 0.8678,
    'v16_baseline_wf_auc': 0.8677,
}
```

### 3-5. WF AUC 詳細

| 年度 | LGB | XGB | ENS |
|------|-----|-----|-----|
| 2021 | 0.8623 | 0.8637 | 0.8638 |
| 2022 | 0.8654 | 0.8662 | 0.8666 |
| 2023 | 0.8668 | 0.8678 | 0.8681 |
| 2024 | 0.8690 | 0.8700 | 0.8703 |
| 2025 | 0.8671 | 0.8678 | 0.8682 |
| **平均** | | | **0.8674** |

**vs V15**: delta = -0.0004 / **vs V16**: delta = -0.0003  
**verdict: NO-GO** (≥V16 0.8677 未達)

### 3-6. lookup tables (推論時の V16a augmentation)

**学習時** → `train/train_v16a_ability.py` が `build_lookup_tables(df)` を呼んで保存  
**ファイル**: `data/v16a_lookup_tables.pkl` (5.2 MB)

```python
tables = {
    'jt':     {(jockey_id, trainer_id): {'top3_rate': float, 'n': int}, ...},  # 31,197 entries
    'perf':   {horse_id: float, ...},                                           # 58,921 entries
    'weight': {horse_id: {'slope': float, 'std': float, 'last': float}, ...},  # 58,921 entries
    'gate':   {(course, dist_bin, surface, umaban): {'top3_rate': float, 'n': int}, ...},  # 1,499 entries
    'built_at': '2026-05-28T11:55:21',
}
```

**推論時** → `tools/features_v16a_lookup.py` の `augment_features_v16a(features_df, race_info)`

```python
@lru_cache(maxsize=1)
def _load_tables() -> dict:
    """data/v16a_lookup_tables.pkl を 1 度だけロード (キャッシュ)"""
    with open(LOOKUP_PATH, 'rb') as f:
        return pickle.load(f)

def augment_features_v16a(features_df, race_info: dict):
    """
    features_df に 7 カラムを追加して返す。
    race_info keys: 'course' (str), 'distance' (int), 'surface' (str)
    """
    tables = _load_tables()
    dist_bin = (int(race_info.get('distance', 0)) // 400) * 400

    rows = []
    for idx, row in features_df.iterrows():
        horse_id   = str(row.get('horse_id', ''))
        jockey_id  = str(row.get('jockey_id', ''))
        trainer_id = str(row.get('trainer_id', ''))
        umaban     = int(row.get('umaban', row.get('馬番', 0)))

        jt  = tables['jt'].get((jockey_id, trainer_id), {})
        wt  = tables['weight'].get(horse_id, {})
        gt  = tables['gate'].get((race_info.get('course',''), dist_bin,
                                  race_info.get('surface',''), umaban), {})

        rows.append({
            'jockey_trainer_top3_rate_exp': jt.get('top3_rate', 0.33),
            'jockey_trainer_n_exp':         float(jt.get('n', 0)),
            'horse_weight_slope_5':         wt.get('slope', 0.0),
            'horse_weight_std_5':           wt.get('std', 5.0),
            'horse_perf_score_exp':         tables['perf'].get(horse_id, 0.33),
            'gate_bias_top3_exp':           gt.get('top3_rate', 0.33),
            'gate_bias_n_exp':              float(gt.get('n', 0)),
        })

    aug = pd.DataFrame(rows, index=features_df.index)
    for col in aug.columns:
        features_df[col] = aug[col]
    return features_df

def invalidate_cache():
    """lookup 更新後に呼ぶ (再ロード強制)"""
    _load_tables.cache_clear()
```

---

## 4. Paper Shadow システム

### 4-1. CANDIDATE_MODELS 一覧

```python
# tools/paper_shadow_v15_full.py
CANDIDATE_MODELS = {
    'v15_full_optuna': 'models/v15_full_optuna_candidate.pkl.gz',   # 3.3 MB
    'v15_2':           'models/v15_2_candidate.pkl.gz',             # 2.6 MB
    # 'v22_top100': DISABLED — track_lap POST-RACE leak 確認済み
    'v20_base':        'keiba_model_v20_base_central.pkl.gz',       # 2.1 MB
    'v16_ability':     'models/v16_ability_candidate.pkl.gz',       # 2.0 MB
    'v16a_ability':    'models/v16a_candidate.pkl.gz',              # 2.1 MB
}
```

### 4-2. 推論フロー

```python
def predict_paper_shadow(features_df, model_key, race_info=None) -> dict:
    """返却: {row_index: score} or None"""
    model = load_candidate_model(model_key)

    # V16 / V16a style: dict with 'model' + 'features'
    if isinstance(model, dict) and 'model' in model and 'features' in model:
        model_feats = model['features']

        if model_key == 'v16a_ability':
            df_aug = _augment_v16a(features_df.copy(), race_info or {})
        else:
            df_aug = features_df

        df_in = df_aug.reindex(columns=model_feats).fillna(0.0)
        X = df_in.values.astype(np.float32)
        p_lgb = model['model'].predict(X)
        w = model.get('ensemble_weights', {'lgb': 0.5, 'xgb': 0.5})
        if 'xgb_model' in model:
            p_xgb = model['xgb_model'].predict(xgb.DMatrix(X, feature_names=model_feats))
            scores = w['lgb'] * p_lgb + w['xgb'] * p_xgb
        else:
            scores = p_lgb

    return {i: float(s) for i, s in enumerate(scores)}
```

### 4-3. 比較・ログ

```python
def run_paper_shadow_comparison(race_id, features_df, v15_predictions, race_info=None):
    """V15 と全 CANDIDATE_MODELS を比較"""
    result = {
        'race_id':    str(race_id),
        'timestamp':  datetime.now().isoformat(),
        'v15_top3':   [p['horse_num'] for p in sorted(v15_predictions,
                       key=lambda x: x['score'], reverse=True)[:3]],
        'paper_shadows': {},
    }
    for model_key in CANDIDATE_MODELS:
        shadow = predict_paper_shadow(features_df, model_key, race_info=race_info)
        if shadow is None: continue
        top3 = sorted(shadow.items(), key=lambda x: x[1], reverse=True)[:3]
        top3_nums = [h for h, _ in top3]
        result['paper_shadows'][model_key] = {
            'top3':          top3_nums,
            'scores':        {str(h): s for h, s in top3},
            'agree_with_v15': set(top3_nums) == set(result['v15_top3']),
        }
    return result

def log_paper_shadow(result, log_dir=None):
    """日次 JSONL: data/paper_shadow_log/shadow_YYYYMMDD.jsonl"""

def get_paper_shadow_stats(days=7) -> dict:
    """返却: {model_key: {'n_agree': int, 'n_total': int}}"""
```

**ログ形式** (`shadow_YYYYMMDD.jsonl`):
```json
{
  "race_id": "202605281201",
  "timestamp": "2026-05-28T09:15:00",
  "v15_top3": [3, 7, 12],
  "paper_shadows": {
    "v16_ability":  {"top3": [3, 7, 11], "agree_with_v15": false},
    "v16a_ability": {"top3": [3, 7, 12], "agree_with_v15": true}
  }
}
```

### 4-4. 呼び出しタイミング

| スクリプト | タイミング | 処理 |
|-----------|-----------|------|
| `tools/daily_predict.py` | 08:00 全レース処理後 | `run_paper_shadow_comparison` → `log_paper_shadow` 各レース。処理完了後に `get_paper_shadow_stats(days=1)` → Discord #updates |
| `tools/race_auto_notify.py` | 発走5分前 各レース | V16 paper log (`data/v16_paper_log/`) + `run_paper_shadow_comparison` → `log_paper_shadow` |

---

## 5. 採用基準・判断フロー

### 5-1. AUC ゲート (WF 検証)

```
V16a WF AUC >= V16 baseline 0.8677  →  AUC GO (本番候補)
V16a WF AUC <  0.8677               →  AUC NO-GO (現状: 0.8674 → NO-GO)
```

### 5-2. LIVE 採用基準 (AUC GO 通過後)

paper shadow で N≥30 レース蓄積後:
1. **人気乖離 ROI > V15** (V15 と異なる上位馬での配当効率)
2. **TOP1 連対率 ≥ 40%** (上位1着予想馬が2着内に入る率)

両方 PASS で本番投票開始 (週末のみ・上限 5,000円/日から段階投入)

### 5-3. 現在の判定

| モデル | AUC ゲート | 状態 |
|--------|-----------|------|
| V16 | PASS (0.8677 ≈ 基準) | paper shadow 継続、LIVE 検証中 |
| V16a | FAIL (0.8674 < 0.8677) | paper shadow のみ、本番採用不可 |

---

## 6. 学習スクリプト

### 6-1. LGB/XGB 共通パラメータ

```python
LGB_PARAMS = {
    'objective': 'binary', 'metric': 'auc', 'boosting_type': 'gbdt',
    'num_leaves': 63, 'learning_rate': 0.05,
    'feature_fraction': 0.8, 'bagging_fraction': 0.8, 'bagging_freq': 5,
    'min_child_samples': 50, 'reg_alpha': 0.1, 'reg_lambda': 0.1,
    'verbose': -1, 'seed': 42,
}
# early_stopping: 50 rounds, max 1000 rounds

XGB_PARAMS = {
    'objective': 'binary:logistic', 'eval_metric': 'auc',
    'max_depth': 6, 'learning_rate': 0.05,
    'subsample': 0.8, 'colsample_bytree': 0.8,
    'min_child_weight': 50, 'reg_alpha': 0.1, 'reg_lambda': 0.1,
    'seed': 42, 'tree_method': 'hist',
}
# early_stopping: 50 rounds, max 1000 rounds
```

### 6-2. WF 検証ループ

```python
WF_YEARS = range(2021, 2026)  # 5-fold

for test_year in WF_YEARS:
    ty = test_year - 2000         # year 列は 2桁 (21, 22, ...)
    train_mask = df['year'] < ty
    test_mask  = df['year'] == ty

    X_tr, y_tr = df[train_mask][features], df[train_mask]['target']
    X_te, y_te = df[test_mask][features],  df[test_mask]['target']

    # LGB
    m_lgb = lgb.train(LGB_PARAMS, lgb.Dataset(X_tr, y_tr),
                      valid_sets=[lgb.Dataset(X_te, y_te)],
                      callbacks=[lgb.early_stopping(50)])
    # XGB
    m_xgb = xgb.train(XGB_PARAMS, xgb.DMatrix(X_tr, y_tr),
                      evals=[(xgb.DMatrix(X_te, y_te), 'valid')],
                      early_stopping_rounds=50)

    auc_ens = roc_auc_score(y_te, 0.5*m_lgb.predict(X_te) + 0.5*m_xgb.predict(...))
```

### 6-3. 学習データキャッシュ

- **ファイル**: `data/_v15_optuna_df_cache.pkl.gz` (104 MB)
- **内容**: `{'df': DataFrame(527,280 rows, 232 cols), 'features': [145 names]}`
- **year 列**: 2桁 (21=2021, 25=2025)
- **target 列**: `finish <= 3` (複勝圏 binary)

### 6-4. 各スクリプト

| スクリプト | 出力 |
|-----------|------|
| `train/train_v15_master.py` | `keiba_model_v15_central.pkl.gz` |
| `train/train_v16_ability.py` | `models/v16_ability_candidate.pkl.gz` |
| `train/train_v16a_ability.py` | `models/v16a_candidate.pkl.gz` + `data/v16a_lookup_tables.pkl` |

---

## 7. 重要ファイルパス

```
keiba-ai/
├── keiba_model_v15_central.pkl.gz        # V15 本番 (145 feats, LGB+XGB)
├── models/
│   ├── v16_ability_candidate.pkl.gz      # V16 候補 (137 feats)
│   ├── v16a_candidate.pkl.gz             # V16a 候補 (144 feats)
│   ├── v15_full_optuna_candidate.pkl.gz
│   ├── v15_2_candidate.pkl.gz
│   └── keiba_model_v20_base_central.pkl.gz
├── data/
│   ├── v16a_lookup_tables.pkl            # V16a 推論用 lookup (5.2 MB)
│   ├── v16_paper_log/                    # V16 日次 paper log (race_auto_notify)
│   ├── paper_shadow_log/                 # 全 CANDIDATE paper shadow JSONL
│   ├── v20/v16a_wf_results.json          # V16a WF 結果
│   └── _v15_optuna_df_cache.pkl.gz       # 学習キャッシュ (104 MB)
├── tools/
│   ├── predict_core.py                   # 特徴量生成・推論エンジン
│   ├── paper_shadow_v15_full.py          # paper shadow 管理
│   ├── features_v16a_lookup.py           # V16a 推論時 augment
│   ├── race_auto_notify.py               # 発走5分前通知 (V16+V16a paper)
│   ├── daily_predict.py                  # 08:00 全レース予測
│   └── daily_results.py                  # 18:00 結果照合
└── train/
    ├── features_v16a.py                  # V16a 特徴量計算 (学習時)
    ├── train_v16_ability.py              # V16 学習
    └── train_v16a_ability.py             # V16a 学習
```

---

## 8. リークフリー設計原則

```python
# Pattern A で除外する特徴量 (8個)
LEAK_FEATURES_A = {
    'odds_log',          # 確定オッズ (投票締切後)
    'horse_weight',      # 当日馬体重 (レース70分前発表)
    'condition_enc',     # 馬場状態 (当日朝発表)
    'weight_change',     # horse_weight 派生
    'weight_change_abs', # horse_weight 派生
    'weight_cat',        # horse_weight 派生
    'weight_cat_dist',   # horse_weight 派生
    'cond_surface',      # condition_enc 派生
}

# SKB POST-RACE LEAK (V20 で追加除外、V15/V16 には含まれていない)
SKB_LEAK_FEATURES = [
    'skb_kishi_code_1', 'skb_kishi_code_2', 'skb_kishi_code_3',
    'skb_baba_code_1',  'skb_baba_code_2',  'skb_baba_code_3',
    'skb_kyaku_code_1', 'skb_kyaku_code_2', 'skb_kyaku_code_3',
    'skb_turf_hoof',
]

# V16a expanding window の必須条件
# 全統計特徴量: cumsum - current (当該行除外) でリーク禁止
# Bayesian smoothing で低サンプル時の過学習を抑制
```

---

## 9. 今後のロードマップ

| 期限 | 作業 | 基準 |
|------|------|------|
| 〜N=30 | V16/V16a paper shadow 蓄積 | `get_paper_shadow_stats()` で確認 |
| N≥30 後 | V16 LIVE 採用判定 | 人気乖離ROI > V15 & TOP1連対率≥40% |
| 6/15+ | V16a 再評価 or V17 設計 | AUC NO-GO なら新特徴量探索 |
| 7/1 | V20 投入候補 | WF AUC ≥ 0.880, JV-Link データ活用 |

---

*このドキュメントは 2026-05-28 時点の実装を反映。コードとの乖離があれば `models/*.pkl.gz` の `features` / `wf_auc_mean` を読んで確認すること。*
