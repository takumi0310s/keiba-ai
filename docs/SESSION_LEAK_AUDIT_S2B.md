...
### 7.8 買い目構造(三連複/三連単フォーメーション)backtest — 全構造NO-GO・現行7点は構造的に妥当 (2026-06-12)
- **問い**: 「切り」と「買い」がモデルで分かるなら、フォーメーションの方が現行7点より効率的では? → `tools/fable_formation_backtest.py`(leak-free v2・V15 OOF・★設計2020-22/評価2023-25・構造数m=6事前固定・同日クラスタbootstrap+Bonferroni α=0.05/6・winsorize99%併記★)
- **土台統計(V15の順位カバレッジ、設計/評価でほぼ同一=安定)**: 馬券内3頭全収納率 top4=29.5%/top5=45.9%/**top6=59.8%**/top8=79.1%。top1馬券内=71.6%・top1勝利=38.2%・勝馬top2内=59.0%。
- **★切り危険の本質★**: レース数ベースで40.2%は馬券内にモデルtop7以下が混入。さらに**払戻金ベースのtop6捕捉率は13.5%しかない**(top8でも29.9%)= 高配当の86%超はモデル圏外から来る。→「切り」を強めても安い的中が残るだけ・「広げ」ても控除率25%に食われる構造。
- **構造別(2023-25, N=10,312R, 100円/点)**:

| 構造 | 点数 | ROI | winsorize床 | 的中率 | maxDD | 対F0差(winsorize, Bonf m=6) |
|------|------|-----|-------------|--------|-------|------------------------------|
| F0 現行7点(基準) | 7 | 156.8% | **149.1%** | 42.0% | ¥25,490 | — |
| F1 ながし10点 | 10 | 152.0% | 139.3% | 47.7% | ¥33,600 | -9.8pt [-15.3,-4.1] **負け確** |
| F2 二軸F 14点 | 14 | 147.4% | 131.8% | 55.3% | ¥50,190 | -17.3pt [-23.8,-10.9] **負け確** |
| F3 切り強化5点 | 5 | 158.9% | 152.1% | 35.7% | ¥39,720 | +3.0pt [-3.1,+9.0] 0跨ぐ |
| F4 三連単F 24点 | 24 | 154.2% | 143.5% | 30.9% | ¥177,450 | -5.6pt [-15.1,+3.9] 0跨ぐ |
| F5 box4点 | 4 | 154.7% | 148.2% | 29.5% | ¥30,170 | -0.9pt [-10.7,+9.0] 0跨ぐ |

- **判定: GO候補なし(全構造で対F0補正後CI生存ゼロ) → paper追加なし・本番不変**。広げる構造(F1/F2)はwinsorize後**有意に悪化**(高配当を拾う頻度よりコスト増が勝つ)。唯一正のF3(切り強化5点)も有意でなく、的中率35.7%への低下でmaxDDがむしろ増(¥25,490→¥39,720)= 資金管理上も改善と言えない。**三連単F4は対F0差が負(δ=-2.7pt)でペア差検証不能(必要N=∞)・控除率27.5%の壁を確認**。
- **結論**: 現行7点フォーメーションは試行6構造中winsorize床で最良。「モデルで切れる」仮説は払戻金捕捉率13.5%が反証 — V15の順位情報は安い的中の予測には有効だが高配当の所在は捕捉できておらず、買い目構造の組み替えでは効率改善しない。改善路線は引き続き戦略層(レース選択=C3/C4型)とモデル層。
- 整合検証: F5(box4点)=154.7% ≈ 公表値V15三連複t4box 154.4%(§2.2、母集団差は tierce払戻要求の-38R)。制約: jra_payouts.csv は現在2018+2023-26のみ(2019-22欠落)→設計期は配当不要のカバレッジ統計のみで設計(look-ahead無し・構造はスクリプト内で事前固定)。

### 7.9 統合タスク 6/12 — 8 DEAD源全修理 + app.py表示修理 + 精度測定 (本番予測ロジック不変、tag pre_fix_0612)
- **8源修理 (降格ゼロ)**: KTA/KKA=fetcher根因修理(直接日付フェッチ新設・3:00 job配線)+バックフィル、TYB/SKB/SRB/SR/CYB/training_eval=再構築。全13 JRDB源 content-fresh(6/7)。詳細=docs/FABLE_SWEEP_LOG.md 統合タスク章
- **app.py (承認済・表示のみ)**: v22(LEAK INVALID)をdiscoveryから除外しPattern A=v15復元 / バッジ動的化(LGB+XGB 2-MODEL・train self-eval表記) / track_record.csv再構築(766R/Σ-29,450=台帳一致) / phase3配線(8戦略paper結果が初稼働)
- **精度測定 (leak-free v2, 2023-25, N=10,364R)**: V15 AUC 0.8418・top1馬券内71.7%・top6収納60.0% / s2b 0.8295・71.2%・58.3% / ★市場(前日人気) 0.7894・63.9%・47.0%★ → V15は市場を top1馬券内+7.7pt/top6収納+13.0pt 超え。s2bのオッズ除去コストは僅少(-0.5pt)
- リハーサル PASS (スコア=predict_core直 43/43・回帰新規failゼロ・Streamlit smoke 200)
