...
既存 tools/v18_v19_retro_no_sib.py 流用、 model dir を v18v19_sib_exp_v1/ に切替 +
...
approach:
  ...
  3. v18_lgb_sib_exp_v1 model で predict (192 features)
...
SIB_EXP_DIR = 'data/v18/v18v19_sib_exp_v1'
OUT_PREDICTIONS = f'{SIB_EXP_DIR}/sib_exp_retro_5_2_5_3_predictions.csv'
...
def load_sib_exp_models():
    v18 = lgb.Booster(model_file=f'{SIB_EXP_DIR}/v18_lgb_sib_exp_v1.txt')
    v19 = lgb.Booster(model_file=f'{SIB_EXP_DIR}/v19_lgb_sib_exp_v1.txt')
    return v18, v19
...
def load_sib_exp_lookup():
    ...
    """
    print("[sib_exp_retro] loading sib_expanding csv ...")
    sib = pd.read_csv("data/netkeiba_siblings_expanding.csv",
                      dtype={'race_id': str, 'horse_id': str})
    print(f"  rows: {len(sib):,}")
    # horse_id 単位 で latest (sib_total_races_exp 最大の row)
    sib_sorted = sib.sort_values(['horse_id', 'sib_total_races_exp'], ascending=[True, False])
...
def main():
    ...
    assert len(sib_exp_in_v18) == 4, f"expected 4 sib_*_exp features, got {sib_exp_in_v18}"
    print("[OK] sib_*_exp 4 features 確認")
