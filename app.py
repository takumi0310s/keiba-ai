...
def load_model():
    import os, gzip
    # v2モデル優先
    for fname in ["keiba_model_v2.pkl.gz", "keiba_model.pkl.gz"]:
        if os.path.exists(fname):
            with gzip.open(fname, "rb") as f:
                data = pickle.load(f)
            if isinstance(data, dict):
                return data['model'], data.get('features', None), data.get('version', 'v1')
            return data, None, 'v1'
    with open("keiba_model.pkl", "rb") as f:
        return pickle.load(f), None, 'v1'
...
model, model_features, model_version = load_model()
