...
def get_horse_stats(horse_id, target_distance, target_surface, target_course=""):
    ...
    try:
        ...
        for ri, row in enumerate(rows):
            ...
            # Date - try multiple patterns including href and 2-digit year
            date_text = tds[0].get_text(strip=True)
            dm = re.search(r'(\d{4})[/\-.](\d{1,2})[/\-.](\d{1,2})', date_text)
            if not dm:
                # Try 2-digit year: "26/03/02"
                dm2 = re.search(r'(\d{2})[/\-.](\d{1,2})[/\-.](\d{1,2})', date_text)
                if dm2:
                    yr = int(dm2.group(1))
                    yr = yr + 2000 if yr < 80 else yr + 1900
                    try:
                        race_dates.append(datetime(yr, int(dm2.group(2)), int(dm2.group(3))))
                    except: pass
                    dm = None  # already handled
                else:
                    # Try from href in first td
                    a_tag = tds[0].find("a")
                    if a_tag:
                        href_text = a_tag.get("href", "")
                        dm = re.search(r'(\d{4})(\d{2})(\d{2})', href_text)
                        if not dm:
                            dm = re.search(r'(\d{4})[/\-.](\d{1,2})[/\-.](\d{1,2})', href_text)
                    if not dm:
                        # Try any td in first 5
                        for td_d in tds[:5]:
                            full_text = td_d.get_text(strip=True)
                            dm = re.search(r'(\d{4})[/\-.](\d{1,2})[/\-.](\d{1,2})', full_text)
                            if dm: break
                            a_t = td_d.find("a")
                            if a_t:
                                dm = re.search(r'(\d{4})(\d{2})(\d{2})', a_t.get("href", ""))
                                if dm: break
            if dm:
                try:
                    ...
                    for td in tds:
                        ...
                        if tm:
                            ...
                            if min_time < secs < max_time and secs < best_time_sec:
                                ...
                                dtm = re.search(r'(\d{4}[/\-.]\d{1,2}[/\-.]\d{1,2})', dt_text)
                                if not dtm:
                                    # 2-digit year
                                    dtm2 = re.search(r'(\d{2})[/\-.](\d{1,2})[/\-.](\d{1,2})', dt_text)
                                    if dtm2:
                                        yr = int(dtm2.group(1))
                                        yr = yr + 2000 if yr < 80 else yr + 1900
                                        best_time_date = f"{yr}/{dtm2.group(2)}/{dtm2.group(3)}"
                                        dtm = True  # mark as found
                                if not dtm:
                                    a_tag = tds[0].find("a")
                                    if a_tag:
                                        href_t = a_tag.get("href", "")
                                        hm2 = re.search(r'(\d{4})(\d{2})(\d{2})', href_t)
                                        if hm2:
                                            best_time_date = f"{hm2.group(1)}/{hm2.group(2)}/{hm2.group(3)}"
                                            dtm = True
                                if not dtm:
                                    for td_d in tds[:5]:
                                        dtm = re.search(r'(\d{4}[/\-.]\d{1,2}[/\-.]\d{1,2})', td_d.get_text(strip=True))
                                        if dtm:
                                            best_time_date = dtm.group(1)
                                            break
                                elif isinstance(dtm, type(re.match('',''))):
