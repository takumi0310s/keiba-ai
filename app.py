    ...
    try:
        ...
        for ri, row in enumerate(rows):
            ...
            for tdi, td in enumerate(tds):
                ...
                # Pass: "3-3-2-1" pattern
                if not found_pass and re.match(r'^\d+-\d+(-\d+)*$', txt):
                    pn = re.findall(r'\d+', txt)
                    if pn and len(pn) >= 2:
                        pass_list.append(int(pn[0]))
                        found_pass = True
                # Agari: float 33.0-44.9
                if not found_agari and re.match(r'^\d{2}\.\d$', txt):
                    try:
                        av = float(txt)
                        if 30.0 < av < 45.0:
...
def parse_shutuba(race_id):
    ...
    for row in rows:
        ...
        # Fallback: try data-id or first td
        if umaban == 0:
            did = row.get("id", "")
            dm = re.search(r'(\d+)$', did)
            if dm:
                umaban = int(dm.group(1)) if int(dm.group(1)) <= 18 else 0
        if umaban == 0:
