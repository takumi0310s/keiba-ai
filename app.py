...
def get_horse_stats(horse_id, target_distance, target_surface, target_course=""):
    ...
    try:
        ...
        for ri, row in enumerate(rows):
            ...
            finish = int(ft)
            # 今日のレース結果かチェック
            row_date_text = tds[0].get_text(strip=True)
            row_is_today = False
            today_str = datetime.now().strftime('%Y/%m/%d')
            today_short = datetime.now().strftime('%Y/%-m/%-d') if hasattr(datetime, 'strftime') else today_str
            if today_str in row_date_text or datetime.now().strftime('%Y/%m/%d') in row_date_text:
                row_is_today = True
            # 日付が今日なら年月日完全一致チェック
            rd_match = re.search(r'(\d{4})/(\d{1,2})/(\d{1,2})', row_date_text)
            if rd_match:
                try:
                    rd_date = datetime(int(rd_match.group(1)), int(rd_match.group(2)), int(rd_match.group(3))).date()
                    if rd_date >= datetime.now().date():
                        row_is_today = True
                except: pass
            if not row_is_today:
                finish_list.append(finish)
                if result['last_finish'] == 5 and not finish_list_has_prev:
                    result['last_finish'] = finish
                    finish_list_has_prev = True
                    if len(tds) > 18:
                        result['margin_text'] = tds[18].get_text(strip=True)
                    if len(tds) > 12:
                        result['prev_jockey'] = tds[12].get_text(strip=True)
            else:
                # 今日のレース結果はスキップ（スコア集計には使わない）
                pass
        ...
        if race_dates:
            ...
            for rd in race_dates:
                if rd.date() < today:
                    ...
                    break
            if prev_date:
                diff = (datetime.now() - prev_date).days
                result['interval_days'] = max(diff, 1)
            elif len(race_dates) >= 2:
                # 全部今日→2番目との差を使う（同日複数出走は稀）
