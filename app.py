...
def get_horse_stats(horse_id, target_distance, target_surface, target_course=""):
    ...
    try:
        ...
        for ri, row in enumerate(rows):
            ...
            # Date - search all tds for valid date pattern
            found_date = False
            for td_idx in range(min(5, len(tds))):
                td_text = tds[td_idx].get_text(strip=True)
                # Pattern 1: "2025/03/02" or "2025-03-02" or "2025.03.02"
                dm = re.search(r'(\d{4})[/\-.](\d{1,2})[/\-.](\d{1,2})', td_text)
                if dm:
                    try:
                        y, m_val, d = int(dm.group(1)), int(dm.group(2)), int(dm.group(3))
                        if 2000 <= y <= 2030 and 1 <= m_val <= 12 and 1 <= d <= 31:
                            race_dates.append(datetime(y, m_val, d))
                            found_date = True
                    except: pass
                if found_date: break
                # Pattern 2: "25/03/02" (2-digit year)
                dm2 = re.search(r'(\d{2})[/\-.](\d{1,2})[/\-.](\d{1,2})', td_text)
                if dm2:
                    try:
                        yr = int(dm2.group(1))
                        yr = yr + 2000 if yr < 80 else yr + 1900
                        m_val, d = int(dm2.group(2)), int(dm2.group(3))
                        if 1 <= m_val <= 12 and 1 <= d <= 31:
                            race_dates.append(datetime(yr, m_val, d))
                            found_date = True
                    except: pass
                if found_date: break
                # Pattern 3: href with "YYYYMMDD" (validate month/day)
                a_tag = tds[td_idx].find("a")
                if a_tag:
                    href_text = a_tag.get("href", "")
                    # Look for date= or YYYYMMDD pattern with valid month
                    hm = re.search(r'(\d{4})(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])', href_text)
                    if hm:
                        try:
                            y, m_val, d = int(hm.group(1)), int(hm.group(2)), int(hm.group(3))
                            if 2000 <= y <= 2030:
                                race_dates.append(datetime(y, m_val, d))
                                found_date = True
                        except: pass
...
st.markdown(f'<div style="text-align:center;margin-top:-12px;margin-bottom:12px"><span class="model-badge {badge_css}">MODEL {model_version.upper()}{auc_text}</span></div>', unsafe_allow_html=True)
...
if st.button("🔍 予想する") and url_input:
    ...
    # Debug: show interval data for first horse
    if horses and horse_ids:
        debug_lines = []
        first_hid = horse_ids[0]
        if first_hid:
            try:
                debug_url = "https://db.netkeiba.com/horse/result/" + first_hid + "/"
                debug_resp = requests.get(debug_url, headers=HEADERS, timeout=10)
                debug_resp.encoding = "EUC-JP"
                debug_soup = BeautifulSoup(debug_resp.text, "html.parser")
                debug_table = debug_soup.find("table", class_="db_h_race_results")
                if debug_table:
                    debug_tbody = debug_table.find("tbody")
                    if debug_tbody:
                        debug_rows = debug_tbody.find_all("tr")
                        if debug_rows:
                            first_row = debug_rows[0]
                            debug_tds = first_row.find_all("td")
                            debug_lines.append(f"馬ID: {first_hid}")
                            debug_lines.append(f"td数: {len(debug_tds)}")
                            for idx, td in enumerate(debug_tds[:8]):
                                txt = td.get_text(strip=True)[:30]
                                a = td.find("a")
                                href = a.get("href","")[:50] if a else "no-link"
                                debug_lines.append(f"td[{idx}]: '{txt}' href='{href}'")
                    else:
                        debug_lines.append("tbody not found")
                else:
                    debug_lines.append("db_h_race_results table not found")
                    # Check for nk_tb_common
                    alt_table = debug_soup.find("table", class_="nk_tb_common")
                    if alt_table:
                        debug_lines.append("Found nk_tb_common table instead")
                    # List all tables
                    all_tables = debug_soup.find_all("table")
                    debug_lines.append(f"Total tables: {len(all_tables)}")
                    for t in all_tables[:5]:
                        cls = t.get("class", [])
                        debug_lines.append(f"  table class: {cls}")
            except Exception as e:
                debug_lines.append(f"Debug error: {str(e)}")
        with st.expander("🔧 Debug: 間隔データ診断"):
    ...
    # === v3専用特徴量 ===
    if model_version == 'v3':
        # 上がり3Fタイム: netkeibaから取得した過去平均上がりを使用
        df['上がり3Fタイム'] = df['上がり3F'].fillna(35.5)

        # 通過順平均: 過去レースの平均通過順位置
        df['通過順平均'] = df['通過順平均'].fillna(8.0)

        # 通過順4: 前走の4コーナー通過順
        df['通過順4'] = df['通過順4'].fillna(8)

        # 人気順: 前走の人気順を使用（今走の人気はレース前には不確定だが、前走人気で代替）
        df['人気順'] = df['前走人気'].fillna(8)

        # オッズlog: 前走オッズのlog変換
        df['オッズlog'] = np.log1p(df['前走オッズ'].clip(1, 999).fillna(15.0))

        # 補正タイム: レース前は不明なのでデフォルト0
        df['補正タイム'] = 0

        # 父馬_enc: sire_mapを使ってエンコード（学習時と同じマッピング）
        ...
        # 上がり順位: レース内での上がり順位（レース前は不明→頭数/2をデフォルト）
        df['上がり順位'] = num_horses / 2
