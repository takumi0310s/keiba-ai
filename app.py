...
def get_horse_stats(horse_id, target_distance, target_surface, target_course=""):
    ...
    try:
        ...
        for ri, row in enumerate(rows):
            ...
            # Pass
            if len(tds) > 20:
                pn = re.findall(r'\d+', tds[20].get_text(strip=True))
                if pn:
                    pass_list.append(int(pn[0]))
            # Agari
            if len(tds) > 22:
                try:
                    av = float(tds[22].get_text(strip=True))
                    if 30.0 < av < 45.0:
...
def parse_shutuba(race_id):
    ...
    dm = re.search(r'(\d{4})m', d01t)
    distance = int(dm.group(1)) if dm else 0
    surface = '芝' if '芝' in d01t else ('ダ' if 'ダ' in d01t else '芝')
...
def render_buy_section(df, race_info, rank_map):
    ...
    n1 = f'{int(t1["馬番"])}{t1["馬名"][:4]}'
    n2 = f'{int(t2["馬番"])}{t2["馬名"][:4]}'
    n3 = f'{int(t3["馬番"])}{t3["馬名"][:4]}'
    # Confidence
    gap12 = t1['スコア'] - t2['スコア']
    ...
    # Hiroku
    himo = []
    if t4 is not None: himo.append(f'{int(t4["馬番"])}')
    if t5 is not None: himo.append(f'{int(t5["馬番"])}')
    if t6 is not None: himo.append(f'{int(t6["馬番"])}')
    himo_str = ''.join([f'{"⑩"[0:0]}{h}' for h in himo]) if himo else ''
    ...
    html += f'<div class="buy-row"><span class="buy-lbl">3連複</span><span class="buy-horses">{int(t1["馬番"])} ― {int(t2["馬番"])},{int(t3["馬番"])} ― {",".join(himo)}</span></div>'
    ...
    if ana_horse is not None:
        ...
        html += f'<div class="buy-row"><span class="buy-lbl">ワイド</span><span class="buy-horses">{ana_num} ― {int(t1["馬番"])},{int(t3["馬番"])}</span></div>'
...
def render_table(df, rank_map):
    ...
    html = '<table class="htable"><tr><th>#</th><th>馬名</th><th>騎手</th><th>脚質</th><th>前走</th><th>間隔</th><th>体重</th><th>SCORE</th></tr>'
...
if st.button("🔍 予想する") and url_input:
    ...
    rank_map, pace_scores_map = calc_pace_advantage(
        race_info['distance'], race_info['surface'], race_info['condition'], num_horses
    )
    ...
    # ===== FINAL: AI 45% + Pop 15% + Apt 10% + Pace 10% + Agari 10% + Course 5% + Other 5% =====
    final_scores = (
