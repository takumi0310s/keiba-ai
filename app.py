...
# 騎手×コース補正係数（主要騎手×主要競馬場）
JOCKEY_COURSE_BONUS = {
    'ルメール':   {'東京':1.20,'中山':1.10,'阪神':1.05,'京都':1.05},
    'C.ルメール': {'東京':1.20,'中山':1.10,'阪神':1.05,'京都':1.05},
    '川田将雅':   {'阪神':1.20,'京都':1.15,'中京':1.10,'東京':0.95},
    '川田':       {'阪神':1.20,'京都':1.15,'中京':1.10,'東京':0.95},
    '武豊':       {'京都':1.30,'阪神':1.15,'東京':1.05},
    '戸崎圭太':   {'東京':1.20,'中山':1.15,'新潟':1.10},
    '横山武史':   {'東京':1.15,'中山':1.10},
    '松山弘平':   {'阪神':1.10,'京都':1.15,'中京':1.05},
    '岩田望来':   {'阪神':1.10,'京都':1.05},
    '坂井瑠星':   {'阪神':1.05,'東京':1.05},
    '福永祐一':   {'京都':1.15,'阪神':1.10},
}

def find_jockey_course_wr(jockey_name, course_name):
    """騎手×コース補正をかけた勝率を返す"""
    base_wr = find_jockey_wr(jockey_name)
    bonus = 1.0
    for key, course_map in JOCKEY_COURSE_BONUS.items():
        if jockey_name.startswith(key) or key.startswith(jockey_name):
            bonus = course_map.get(course_name, 1.0)
            break
    return min(base_wr * bonus, 0.40)  # 上限40%
...
def get_horse_stats(horse_id, target_distance, target_surface, target_course=""):
    result = {
        ...
        'prev2_margin': 0.0, 'prev3_margin': 0.0,
        'finish_trend': 0,  # 正=改善傾向, 負=悪化傾向
        'top3_count_3r': 0,  # 過去3走で3着内の回数
        'jockey_course_wr': 0.08,  # 騎手×コース別勝率（後で上書き）
        'surface_dist_enc': 0,  # 馬場×距離交差
    }
    try:
        ...
        # === 過去3走追加特徴量 ===
        if len(finish_list) >= 2:
            result['prev2_finish'] = finish_list[1]
        if len(finish_list) >= 3:
            result['prev3_finish'] = finish_list[2]
        # トレンド: 最新着順 - 2走前着順（負=改善）
        if len(finish_list) >= 2:
            result['finish_trend'] = finish_list[0] - finish_list[1]
        # 過去3走で3着内の回数
        result['top3_count_3r'] = sum(1 for f in finish_list[:3] if f <= 3)
...
def parse_shutuba(race_id, is_nar=False):
    ...
    for row in rows:
        ...
        horses.append({
            ...
            '騎手コース勝率': find_jockey_course_wr(jockey_name, course_name),
...
if st.button("🔍 予想する") and url_input:
        ...
        for i, (horse, hid) in enumerate(zip(horses, horse_ids)):
            if hid:
                try:
                    ...
                    # === 過去3走追加特徴量 ===
                    horse['2走前着順'] = stats.get('prev2_finish', 5)
                    horse['3走前着順'] = stats.get('prev3_finish', 5)
                    horse['着順トレンド'] = stats.get('finish_trend', 0)
                    horse['3走内3着回数'] = stats.get('top3_count_3r', 0)
                except Exception:
    ...
    if model_version in ('v5', 'v6'):
        ...
        # === 改善①: 過去3走特徴量 ===
        df['prev2_finish'] = df.get('2走前着順', pd.Series([5]*len(df))).fillna(5)
        df['prev3_finish'] = df.get('3走前着順', pd.Series([5]*len(df))).fillna(5)
        df['finish_trend'] = df.get('着順トレンド', pd.Series([0]*len(df))).fillna(0)
        df['top3_count_3r'] = df.get('3走内3着回数', pd.Series([0]*len(df))).fillna(0)
        # avg_finish_3r: 3走平均着順（より精密に計算）
        df['avg_finish_3r'] = (df['前走着順'].fillna(5) + df['prev2_finish'] + df['prev3_finish']) / 3.0
        df['avg_finish_5r'] = df['avg_finish_3r']  # 5走分は取れないので3走平均で代用
        df['avg_last3f_3r'] = df['上がり3F'].fillna(35.5)
        df['best_finish_5r'] = df[['前走着順', 'prev2_finish', 'prev3_finish']].min(axis=1).fillna(5)
        ...
        # === 改善②: 騎手×コース別勝率 ===
        if '騎手コース勝率' in df.columns:
            df['jockey_course_wr'] = df['騎手コース勝率'].fillna(df['騎手勝率'])
        else:
            df['jockey_course_wr'] = df['騎手勝率']
        df['jockey_dist_wr'] = df['騎手勝率']  # 距離別は全体勝率で代用
        df['jockey_top3'] = df['jockey_course_wr'] * 3.0  # top3率の近似

        # === 改善③: 調教師勝率（デフォルト値のまま、将来拡張用） ===
        df['trainer_wr'] = 0.08
        df['trainer_top3'] = 0.25

        # === 改善⑤: 馬場×距離交差特徴量 ===
        df['surface_dist_enc'] = df['芝ダート_enc'] * 10 + df['dist_cat'].astype(float).fillna(2)
